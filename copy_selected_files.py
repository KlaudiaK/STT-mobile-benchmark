...
def process_subset(input_folder, subset_csv, output_folder, flat_structure=True):
    ...
    # Load subset
    subset = pd.read_csv(subset_csv, dtype={
        'filename': str,
        'speaker_id': str,
        'chapter_id': str,
        'sex': str,
    ...
    for _, row in subset.iterrows():
        ...
        trans_src = os.path.join(input_folder, speaker_id, chapter_id, 
    ...
    with open(os.path.join(output_folder, "combined_transcriptions.txt"), 'w', encoding='utf-8') as f:
        f.write("\n".join(transcriptions))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--subset", default="selected_subset.csv", help="Subset CSV file")
    parser.add_argument("--output", default="selected_audio", help="Output folder path")
